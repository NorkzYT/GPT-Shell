    ...
    private async SendPromptToGPTChat(
        ...
        while (!finished) {
            ...
            try {
                ...
                const currentResponseTokens = encode(completeResponseText).length;
...
// ${prompt.split('\n').map(item => `> ${item}`).join('\n')}`)
                ...
                logMessage(`Prompt is using ${messages.length} messages out of ${this.messageHistory.length} for ${encode(prompt).length} tokens.`);
